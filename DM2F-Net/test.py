...
args = {
    ...
    'snapshot': 'iter_6000_loss_0.05102_lr_0.000000',
}
        ...
        for name, root in to_test.items():
            ...
            for idx, data in enumerate(dataloader):
                ...
                for i in range(len(fs)):
                    ...
                    ssims.append(ssim)
                    print('predicting for {} ({}/{}) [{}]: PSNR {:.4f}, SSIM {:.4f}, MSE {:.4f}'
                          .format(name, idx + 1, len(dataloader), fs[i], psnr, ssim,mse))
            ...
            print(f"[{name}] L1: {loss_record.avg:.6f}, PSNR: {np.mean(psnrs):.6f}, SSIM: {np.mean(ssims):.6f},MSE: {np.mean(mses):.6f}")
